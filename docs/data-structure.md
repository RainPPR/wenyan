# 数据结构说明 (Data Structure Specification)

数据去重与结构设计原则：
项目对文言词条进行整理归纳，合并同一词语的多来源异义项与课文例句，保持数据准确与务实：

1. **分类划分标准与数据文件**：
   - 高频实词与难词精选：划入模块化的 `/src/data/shiciPart1.ts` ~ `/src/data/shiciPart5.ts`，共计150+重点实词与高频阅读短语。
   - 核心文言虚词：划入 `/src/data/xuci.ts`（涵括“而、何、乎、乃、其、且、若、所、为、焉、也、以、因、于、与、则、者、之”18大核心虚词的完整义项、词性划分、复音虚词、高中课文权威例句与现代汉语精准译文）。
   - 高频难词与固定结构：与实词集深度打通合并，并通过 `/src/data/nanci.ts` 导出供分类筛选使用。
   - 移除原先硬编码的固定数量限制（如“120实词”、“18虚词”等描述），统一归纳为高频重点文言词条。

2. **VocabularyEntry 统一字段接口**：
   - `id`: 唯一标识符 (通过全局映射保障 ID 严格唯一，彻底消除重复 Key 警告)
   - `word`: 汉字字词（如 "安", "而", "苟", "孰与", "甫"）
   - `pinyin`: 拼音标注 (支持多音字字符串或数组，如 "fǔ", "cháo / zhāo")
   - `category`: 分类 (`'shici_120'` | `'xuci_18'` | `'gaopin_nanci'` | `'guding_jushi'`)
   - `categoryLabel`: 分类中文名称（如 "高频文言实词", "高频文言虚词", "高频难词与短语"）
   - `radical`: 部首/笔画参考 (可选)
   - `isHighFrequency`: 是否高频常见词 (布尔值)
   - `senses`: 义项列表 `SenseItem[]`
     - `pos`: 词性 (`动词` | `名词` | `形容词` | `副词` | `介词` | `连词` | `代词` | `兼词` 等)
     - `meaning`: 释义说明
     - `examples`: 例句数组 `ExampleItem[]`
       - `text`: 文言原文
       - `source`: 课文出处（如《鸿门宴》《廉颇蔺相如列传》《过秦论》《师说》）
       - `translation`: 现代汉语翻译
   - `examTips`: 文言用法与常见易错点提示
